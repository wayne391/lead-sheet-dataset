...
def chord_parser(chord, mode, key_offset):
    ...
    chord_key = borrowed if borrowed else MODE_TO_KEY[int(mode)]
    ...
    if 'emb' in chord:
        emb = chord['emb']
        comp_vec = set_emb(comp_vec, scale, sd, emb)
...
def reset_chord_basic(data, comp_vec):
    ...
    use this function to update th chord object to avoind
...
def proc_roman_to_symbol(raw, is_key=True, save_path=None, name='tab', save_type='pickle'):
    ...
    # overwrite roman
    raw['tracks']['melody'] = melody_events
    raw['tracks']['chord'] = chord_events
    ...
    if save_path:
        ...
        if save_type is 'pickle':
            ...
            with open(file_name, 'w') as handle:
                json.dump(raw, handle, cls=MyEncoder)
        else:
...
if __name__ == '__main__':
    file_path = 'datasets/xml/x/xxanaxx/story/pre-chorus-and-chorus.xml'
    ...
    cidx = 0
    print('> Input:')
    pprint.pprint(raw_roman['tracks']['chord'][cidx])

    print('> Result:')
    pprint.pprint(raw_symbol['tracks']['chord'][cidx])

    print('> Symbol')
    print(raw_symbol['tracks']['chord'][cidx]['symbol'])
    ...
    # nidx = 0
    # print('> Input:')
    # pprint.pprint(raw_roman['tracks']['melody'][nidx])

    # print('> Result:')
    # pprint.pprint(raw_symbol['tracks']['melody'][nidx])
